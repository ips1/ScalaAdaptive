...
/**
  ...
  * measurements ([[scalaadaptive.core.runtime.history.runhistory.RunHistory]]) and an input descriptor. The goal is to
  * select the function with best expected performance on given input. The implementation can use any approach in the
  * selection process and can optimize using different observations included in TMeasurement.
  *
...
trait SelectionStrategy[TMeasurement] {
  /**
    * Selects an option to run given a set of set of historical measurement sequences (
    * [[scalaadaptive.core.runtime.history.runhistory.RunHistory]]), one for each function. Can optimize for various
    * aspects.
    *
