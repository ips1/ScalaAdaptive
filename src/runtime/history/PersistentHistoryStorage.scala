...
/**
  ...
  */
class PersistentHistoryStorage[TMeasurement](private val mapHistory: MapHistoryStorage[TMeasurement],
  ...
  override def getHistory(key: HistoryKey): RunHistory[TMeasurement] = {
    if (mapHistory.hasHistory(key)) {
      return mapHistory.getHistory(key)
    }
    val loaded = historySerializer.deserializeHistory(key)

    loaded match {
      case Some(history) => {
        mapHistory.addHistory(history.key, history)
        return history
      }
      case _ => mapHistory.getHistory(key)
    }
  }

  override def applyNewRun(key: HistoryKey, run: RunData[TMeasurement]): Unit = {
    historySerializer.serializeNewRun(key, run)
    mapHistory.applyNewRun(key, run)
  }
}
